...
{
    FIT_DATA_TYPE **x = initialization(pop, lb, ub);      // pop initialization
        ...
        for (int i = 0; i < pop; i++)
        {
            ...
            else
            {
                ...
                for (int j = 0; j < DIM; j++)
                {
                    ...
                    if (r < p)
                    {
                        x[i][j] = bestPositions[j] + vb[j] * (W[i][j] * x[A][j] - x[B][j]);
                    }
                    else
                    {
                        x[i][j] = vc[j] * x[i][j];
